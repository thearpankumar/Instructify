...
class ChatHandler:
    ...
    async def handle_message(self, class_id: str, websocket: WebSocket, message_data: dict):
    ...
    async def handle_chat_message(self, class_id: str, websocket: WebSocket, message_data: dict):
        ...
        if sender_type == "student":
            try:
                ...
                classification = await self.ollama_client.classify_doubt(message_text, context)
                ...
                if is_doubt:
                    ...
                    await self.room_manager.send_to_teachers(class_id, {
                        "type": "doubt_notification",
                        "student_name": sender_name,
                        "message": message_text,
                        "confidence": classification.get("confidence", 0.5),
                        "reason": classification.get("reason", "Classified as academic question"),
        ...
        await self.room_manager.broadcast_to_room(class_id, {
            "type": "chat_message",
            "id": chat_message.id,
    ...
    async def handle_ai_query(self, class_id: str, websocket: WebSocket, message_data: dict):
        ...
        try:
            ...
            ai_response = await self.ollama_client.answer_student_query(query, lecture_context)
            ...
            if ai_response:
                ...
                await websocket.send_text(json.dumps({
                    "type": "ai_response",
                    "query": query,
                    "response": "Sorry, I'm having trouble processing your question right now. Please try again or ask your teacher directly.",
                    "timestamp": datetime.now().isoformat()
                }))
        except Exception as e:
    ...
    async def handle_webrtc_signal(self, class_id: str, websocket: WebSocket, message_data: dict):
        ...
        signal_data = {
            ...
            "from": self.room_manager.user_connections.get(websocket, {}).get("user_name", "Anonymous")
        }
        
        await self.room_manager.broadcast_to_room(class_id, signal_data, exclude=websocket)
